...
void main() {
	...
	// Specular
	float spec1 = pow(max(dot(N, lightDir1), 0.0), inSpecStrength.y); // Shininess coefficient (can be a uniform)
	vec3 specular1 = inSpecStrength.x * spec1 * lightColour; // Can also use a specular color
	float spec2 = pow(max(dot(N, lightDir2), 0.0), inSpecStrength.y); // Shininess coefficient (can be a uniform)
	vec3 specular2 = inSpecStrength.x * spec2 * lightColour; // Can also use a specular color

	vec4 textureColour = texture(s_texture, inUV);
	vec3 result = (ambient + diffuse + specular1 + specular2) * inColour * textureColour.rgb;
