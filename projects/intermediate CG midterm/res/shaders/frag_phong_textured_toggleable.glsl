...
void main() {
	...
	if (u_usingSpecular) {
		...
		result += (u_SpecularLightStrength * texSpec * spec); // Can also use a specular color
	}

	if (u_usingAmbient) {
		frag_color = vec4((
			(u_AmbientCol * u_AmbientStrength) +	// global ambient light
			result * u_LightCol * attenuation		// light factors from our single light
